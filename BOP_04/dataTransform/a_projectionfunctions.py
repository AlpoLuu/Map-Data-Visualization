...
def robinsonArray(array): #robinson could be problematic for millions of values, may be omitted
    ...
    array[:,CIs["x_rob"]] = array[:,CIs["x_rob"]]
    #print(array[:,CIs["x_rob"]])

    array[:,CIs["x_rob"]] = 0.8487 * rLF_valuesX * ( array[:,CIs["x_rob"]] / radToDeg )
...
def robinsonLookupFunction(array, XY):
    ...
    if(XY == 0):
        ...
        x_values = -5.83803168/np.power(10,7)*np.power(x_values,3) + 4.9463154/np.power(10,5)*np.power(x_values,2) + 1.13882218008/np.power(10,2)*np.power(x_values,1)
    ...
    if(XY == 1):
        ...
        y_values = 1.25492697/np.power(10,7)*np.power(y_values,3) + -7.97357073825/np.power(10,5)*np.power(y_values,2) + 9.61931994525/np.power(10,4)*np.power(y_values,1)
