...
def dsToZoom(arrowArr, downscaleAmount, inColumn, filterItem): #downsamples a pyarrow table according to zoom of program
    #1/4 resolution or compression for
    df = arrowArr.with_row_index("__idx")

    map_rows = df.filter(pl.col(inColumn) == filterItem).gather_every(downscaleAmount)
