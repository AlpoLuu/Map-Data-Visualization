...
void mapImage(String[][] stringColumns, float[][] floatColumns) {
  print("lol");
  for (int i = 0; i < 3_500_000; i = i + 1) { // there's 3.5 million rows to check and append to maphits
  ...
  if (toMakeImage == 0) {
    ...
    mapImageToDisplay = createImage(540, 1080, RGB); //map size is 1080 * 540 as image
    mapImageToDisplay.loadPixels();
    
    // x = 0, y = 1, 2, . . . . 1080; x = 1, y = 1 2, . . . , 1080, . . . x = 540

    for ( int i = 0; i < mapHits.size(); i++) {
      if ( floatColumns[10][mapHits.get(i)] == 1) {
        mapImageToDisplay.pixels[i] = LAND;
      } else {
        mapImageToDisplay.pixels[i] = SEA;
      }
    }
